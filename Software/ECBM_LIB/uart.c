...
#endif
#else                                         //若没有打开校验或者多机通信，
#if ((ECBM_UART1_SCON_CONFIG & 0x80) == 0x80) //却设置成了9位数据。
#warning 串口1当前没有开启校验功能，请留意一下工作模式是否一定设置为9位。
#endif
#endif
#endif
...
#endif
#else                                          //若没有打开校验或者多机通信，
#if ((ECBM_UART2_S2CON_CONFIG & 0x80) == 0x80) //却设置成了9位数据。
#warning 串口2当前没有开启校验功能，请留意一下工作模式是否一定设置为9位。
#endif
#endif
#endif
...
#endif
#else                                          //若没有打开校验或者多机通信，
#if ((ECBM_UART3_S3CON_CONFIG & 0x80) == 0x80) //却设置成了9位数据。
#warning 串口3当前没有开启校验功能，请留意一下工作模式是否一定设置为9位。
#endif
#endif
#endif
...
#endif
#else                                          //若没有打开校验或者多机通信，
#if ((ECBM_UART4_S4CON_CONFIG & 0x80) == 0x80) //却设置成了9位数据。
#warning 串口4当前没有开启校验功能，请留意一下工作模式是否一定设置为9位。
#endif
#endif
#endif
//------------------------------------串口数量警告---------------------------------//
...
#endif
//----------------------------------和定时器冲突警告-------------------------------//
#if (ECBM_UART1_EN) && ((ECBM_UART1_AUXR_CONFIG & 0x01) == 0x01) && (ECBM_UART2_EN)
#if (ECBM_UART1_BAUD_RATE != ECBM_UART2_BAUD_RATE)
#error 串口1和串口2的波特率产生器（定时器2）冲突。请更换串口1的波特率产生器或者将串口1和串口2的波特率调成一致。
#endif
#endif
#if (ECBM_UART1_EN) && ((ECBM_UART1_AUXR_CONFIG & 0x01) == 0x01) && (ECBM_UART3_EN) && ((ECBM_UART3_S3CON_CONFIG & 0x40) == 0x00)
#if (ECBM_UART1_BAUD_RATE != ECBM_UART3_BAUD_RATE)
#error 串口1和串口3的波特率产生器（定时器2）冲突。请更换串口1或串口3的波特率产生器或者将串口1和串口3的波特率调成一致。
#endif
#endif
#if (ECBM_UART1_EN) && ((ECBM_UART1_AUXR_CONFIG & 0x01) == 0x01) && (ECBM_UART4_EN) && ((ECBM_UART4_S4CON_CONFIG & 0x40) == 0x00)
...
void uart_init(void)
{
    ...
    UART1_SET_REG_SCON(ECBM_UART1_SCON_CONFIG); //写入SCON配置，包含工作模式、接收使能的选项。
    UART1_SET_REG_AUXR(ECBM_UART1_AUXR_CONFIG); //写入AUXR配置，包含波特率发生器选择。
    ...
    UART2_SET_REG_S2CON(ECBM_UART2_S2CON_CONFIG); //写入S2CON配置,包含了通信位数、接收允许。
    ...
    UART3_SET_REG_S3CON(ECBM_UART3_S3CON_CONFIG); //写入S3CON配置,包含了通信位数、接收允许、波特率产生器选择。
    ...
    UART4_SET_REG_S4CON(ECBM_UART4_S4CON_CONFIG); //写入S4CON配置,包含了通信位数、接收允许、波特率产生器选择。
...
-------------------------------------------------------*/
#if ECBM_UART1_EN
void uart1_it(void) UART1_IT_NUM
{
...
-------------------------------------------------------*/
#if ECBM_UART2_EN
void uart2_it(void) UART2_IT_NUM
{
...
-------------------------------------------------------*/
#if ECBM_UART3_EN
void uart3_it(void) UART3_IT_NUM
{
...
-------------------------------------------------------*/
#if ECBM_UART4_EN
void uart4_it(void) UART4_IT_NUM
{
